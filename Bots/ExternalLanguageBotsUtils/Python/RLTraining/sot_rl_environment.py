...
class TalesOfTribute(gym.Env):
    ...
    def step(self, action):
        ...
        if decoded_message[0] == "Gamestate":
            ...
            #file_name = 'Bots\\ExternalLanguageBotsUtils\\Python\\RLTraining\\Winners_Log.txt'
...
def run_evaluation_games(direct, opponent):
    ...
    RL_folder = 'Bots\\ExternalLanguageBotsUtils\\Python\\RLTraining'
    os.chdir(RL_folder)
...
if __name__ == "__main__":
    ...
    # todo: you can load a model from a file, e.g.:
    # model = PPO.load(f"{models_dir}/{date}/..._training_iterations.zip", env, verbose=1, tensorboard_log=logdir, device="cuda")
